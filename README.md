...
1. [draw.io](https://app.diagrams.net/) をブラウザで開くか、デスクトップ版を起動します。
   ...
   - **※Windows デスクトップ版で `.drawio` ファイルの関連付けから起動する場合:**  
     ファイルをダブルクリックして開く場合もプラグインを有効にするには、関連付けの `open` コマンドに `--enable-plugins` を追加します。まず `.drawio` の ProgID を確認します。

     ```cmd
     reg query HKCR\.drawio /ve
     ```

     代表例では、既定値が `draw.io Diagram` になっています。その場合、実際の起動コマンドは以下で確認できます。
